...
impl<T: Clone> TArray<T> {
    pub fn new() -> Self {
        Self { data: std::ptr::null(), count: 0u32, max: 0u32 }
    }
    ...
    pub fn iter(&self) -> TArrayIterator<T> {
        TArrayIterator { array: self, index: 0 }
    }
}
...
impl FString {
    pub fn new(string: &str) -> Self {
        ...
        Self(TArray { data: utf16.as_ptr(), count: utf16.len() as u32, max: utf16.len() as u32 })
    }
}
...
pub struct TUObjectArray {
    ...
    // 0x00(0x08)
    pre_allocated_objects: *const FUObjectItem,
    // 0x08(0x08)
