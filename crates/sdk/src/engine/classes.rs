...
use crate::core::structs::{FLinearColor, FVector, FVector2D};
...
impl AActor {
    pub fn get_distance_to(&self, actor: *const AActor) -> f32 {
        ...
        let mut params = Params { other_actor: actor, return_val: 0f32, pad_0c: [0u8; 0x4] };
...
static K2_TEXT_SIZE: OnceLock<usize> = OnceLock::new();
static K2_STR_LEN: OnceLock<usize> = OnceLock::new();
static K2_PROJECT: OnceLock<usize> = OnceLock::new();
static K2_DRAW_TEXT: OnceLock<usize> = OnceLock::new();
static K2_DRAW_LINE: OnceLock<usize> = OnceLock::new();
static K2_DRAW_BOX: OnceLock<usize> = OnceLock::new();

impl UCanvas {
    pub fn k2_text_size(&self, font: *const UFont, text: &str, scale: &FVector2D) -> FVector2D {
        #[repr(C)]
        struct Params {
            render_font: *const UFont,
            render_text: FString,
            scale: FVector2D,
            return_val: FVector2D,
        }
        ...
        let mut params = Params {
            ...
            scale: *scale,
            return_val: FVector2D { x: 0f32, y: 0f32 },
        };

        K2_TEXT_SIZE.get_or_init(|| {
            UObject::find_object::<usize>("Function Engine.Canvas.K2_TextSize") as usize
        });

        if let Some(text_size_fn) = K2_TEXT_SIZE.get() {
            self.object_.process_event(
                *text_size_fn as *const UFunction,
                &mut params as *mut _ as *mut usize,
            );
        }
    ...
    pub fn k2_str_len(&self, font: *const UFont, text: &str) -> FVector2D {
        #[repr(C)]
        struct Param {
            font: *const UFont,
            text: FString,
        ...
        let mut param =
            Param { font, text: FString::new(text), return_val: FVector2D { x: 0f32, y: 0f32 } };

        K2_STR_LEN.get_or_init(|| {
            UObject::find_object::<usize>("Function Engine.Canvas.K2_StrLen") as usize
        });

        if let Some(str_len_fn) = K2_STR_LEN.get() {
            self.object_
                .process_event(*str_len_fn as *const UFunction, &mut param as *mut _ as *mut usize);
        }

        param.return_val
    }

    pub fn k2_project(&self, world_position: &FVector) -> FVector {
        #[repr(C)]
        struct Params {
        ...
        let mut params = Params { world_position: *world_position, return_val: FVector::zero() };

        K2_PROJECT.get_or_init(|| {
            UObject::find_object::<usize>("Function Engine.Canvas.K2_Project") as usize
        });

        if let Some(project_fn) = K2_PROJECT.get() {
            self.object_.process_event(
                *project_fn as *const UFunction,
                &mut params as *mut _ as *mut usize,
            );
        }
    ...
    pub fn k2_draw_text(
        ...
        K2_DRAW_TEXT.get_or_init(|| {
            UObject::find_object::<usize>("Function Engine.Canvas.K2_DrawText") as usize
        });

        if let Some(draw_text_fn) = K2_DRAW_TEXT.get() {
            self.object_.process_event(
                *draw_text_fn as *const UFunction,
                &mut params as *mut _ as *mut usize,
            );
        }
    }
    ...
    pub fn k2_draw_line(
        ...
        K2_DRAW_LINE.get_or_init(|| {
            UObject::find_object::<usize>("Function Engine.Canvas.K2_DrawLine") as usize
        });

        if let Some(draw_line_fn) = K2_DRAW_LINE.get() {
            self.object_.process_event(
                *draw_line_fn as *const UFunction,
                &mut params as *mut _ as *mut usize,
            );
        }
    }
    ...
    pub fn k2_draw_box(
        ...
        let mut params = Params { position: *position, size: *size, thickness, color: *color };

        K2_DRAW_BOX.get_or_init(|| {
            UObject::find_object::<usize>("Function Engine.Canvas.K2_DrawBox") as usize
        });

        if let Some(draw_box_fn) = K2_DRAW_BOX.get() {
            self.object_.process_event(
                *draw_box_fn as *const UFunction,
                &mut params as *mut _ as *mut usize,
            );
        }
    }
...
impl UGameplayStatics {
    ...
    pub fn spawn_object(&self, class: *const UClass, outer: *const UObject) -> *const UObject {
        ...
        unsafe {
            ...
            let mut params = SpawnObjectParams { class, outer, return_val: std::ptr::null_mut() };
...
/// Size -> 0x0328 (FullSize[0x0360] - InheritedSize[0x0038])
#[repr(C)]
...
pub struct ULevel {
    ...
    pad_278: [u8; 0x10],
    // 0x278(0x10)
    destroyed_replicated_static_actors: [u8; 0x10],
    // 0x288(0x10)
    // TArray<FReplicatedStaticActorDestructionInfo>
    // destroyed_replicated_static_actors
}
